...
- **Part C 以降** — URL/arXiv ingest、PDF page 抽出、OCR、Claim 抽出、Bundle、lifecycle、registry、notebook 管理、ソース一覧・横断検索、暗号化・identity・メール、Claude Code セッションログまで。
...
> **メモ (auto-trigger scheduler の自動起動):** SourceVault のロード時に、この PC が auto-trigger ジョブ (「run this workflow on <PC>」のような他 PC からの job も含む) を **常に拾えるよう**、auto-trigger scheduler が自動起動されます。ただし scheduler は **1 マシンにつき 1 か所だけ = 対話的な FrontEnd カーネル (`$FrontEnd =!= Null`) でのみ起動**します。headless カーネル (subkernel、external-job wolframscript、サービスカーネル、MCP gateway 等) は `$FrontEnd === Null` なので除外され、ライセンスシート数の爆発と dispatch の多重化を防ぎます。
>
> - 自動起動は **冪等** です (`SourceVaultAutoTriggerStartScheduler[]` が同じ tick を再登録するだけ)。
> - claudecode / autotrigger がまだロードされていない場合、`SourceVaultAutoTriggerStartScheduler` は安価な no-op で `ClaudeCodeAbsent` を返し、次回 SourceVault (re)load 時に改めて起動を試みます。
> - 結果は `SourceVault\`Private\`$iSVAutoTriggerSchedulerAutoStartResult` に記録されます (`Status` は起動成功のほか、`Skipped`/`AutoTriggerUnavailable`、`Skipped`/`DisabledByUser`、`Skipped`/`NotFrontEndKernel`、`Failed`/`AutoStartException` 等)。
> - 自動起動を抑止したい場合は、**ロード前**に `SourceVault\`Private\`$iSVDisableAutoTriggerScheduler = True` を設定します。
>
> scheduler は claudecode の共有 polling tick に相乗りするだけなので、追加のコストはほぼありません。
...
>
> なお、メタデータ付きで一覧表示したい場合は **`SourceVaultSources` / `SourceVaultArXiv` / `SourceVaultSummaries` / `SourceVaultClaudeCode`** (Part M / Part O 参照) を使うと、タイトル・著者・要約・URL リンク付きの表で俯瞰できます。
...
> **メモ (バージョン番号のソートキー修正):** model registry の内部では、`{4, 6}` のようなバージョン番号を `SortBy` 用の単調な数値キーに変換する `iSVParseModelVersion` が使われます。旧実装は指数に `Length[v]` を用いていたため、**桁数の異なるバージョン間で逆転**が起こり、`claude-sonnet-4-6` ({4,6}=4006) が `claude-sonnet-5` ({5}=5) に「桁数の少ない新メジャー版が勝ってしまう」問題がありました (2026-07-06 に LM Studio モデルが誤ルートした実例への対処)。現在は各桁を固定幅 (width 6、基数 100000) まで右パディングしてから重み付けするため、`{4,6}` → 4000600000 のように **常に桁上がりし、正しく新版が上位**になります。日付は `iSVParseModelVersion` で除外済み (<10000 より大きいので桁上がりしない)。
...
# Part O. Claude Code セッションログ (llmlog)

補助パッケージ `SourceVault_llmlog` により、**Claude Code の実行ログ / セッションログ / 作業ログ**を SourceVault の first-class ソースとして取り込み、検索・共有できます。他の種別ビュー (arXiv / Eagle / mail) と同じ「種別専用ビュー + 横断検索 provider」の枠組みに乗っています。

ストーリー:

> 「Claude Code で行った過去の作業セッションのログを SourceVault に取り込んで、あとで『あのとき何をしたか』を検索したい。GitHub のコミット履歴 (GitHubCommitLog) やリポジトリ検索とは別物として、Claude Code のログだけを引きたい。」
...
| 機能 | 用途 |
|---|---|
| `SourceVaultIngestClaudeCodeLogs[opts]` | Claude Code のセッションログを SourceVault に取り込む |
| `SourceVaultClaudeCode[query, opts]` | 取り込んだセッションログだけを共通スキーマ表で表示 (種別専用ビュー) |

`SourceVaultClaudeCode` は `SourceVaultArXiv` / `SourceVaultEagleSummaries` / `SourceVaultMailSearchSummary` と同じ「種別専用ビュー」で、リンク開き・絞り込み検索を持ち、横断検索 `SourceVaultSummaries` にも `"llmlog"` provider として相乗りします。すべての公開シンボルは context `SourceVault\`` にあります。

---

## 例 O-1: Claude Code ログを取り込む

```mathematica
(* Claude Code のセッションログ (claudecode_sessions) を取り込む *)
SourceVaultIngestClaudeCodeLogs[]
(* → <|"Status" -> "OK", "Ingested" -> _Integer, ...|> *)
```

取り込まれた各セッションは snapshot 化され、通常のソースと同様にハッシュで固定・dedup されます。
...
```mathematica
SourceVaultClaudeCode[""]            (* 取り込み済み全セッションログ *)
SourceVaultClaudeCode["SourceVault"] (* キーワード部分一致 *)
SourceVaultClaudeCode["", "On" -> Today]
```

`SourceVaultClaudeCode[query]` は Claude Code セッションログだけを共通スキーマ表で表示します。オプションは `SourceVaultSources` と同じ (`"On"` / `"Since"` / `"Until"` / `"Limit"` / `"Format"` 等)。

---

## 例 O-3: 横断検索での provider 分離 (GitHub コミット履歴と混同させない)

`SourceVaultSummaries` の横断検索では、Claude Code セッションログは `"llmlog"` provider として統合されます。

```mathematica
(* llmlog provider だけに絞って横断検索 *)
SourceVaultSummaries["リファクタ", "Providers" -> {"llmlog"}]

(* sources + eagle + llmlog をまとめて検索 *)
SourceVaultSummaries["リファクタ"]
```

> **メモ (GitHubCommitLog / GitHub リポジトリ検索との区別):** 「Claude Code のログ」を **GitHub のコミット履歴 (GitHubCommitLog) や GitHub リポジトリ検索と混同させない**よう、llmlog provider は `"Claude Code"` / `"ClaudeCode"` / `"実行ログ"` / `"セッションログ"` / `"作業ログ"` / `"過去のセッション"` / `"svcclog"` / `"SourceVaultClaudeCode"` / `"claudecode_sessions"` などの語彙で明確にルーティングされます (bare `"ログ"` は over-match するのでルーティング語彙には含めません)。「Claude Code のログ」を検索したいときは `SourceVaultClaudeCode` / `SourceVaultIngestClaudeCodeLogs` を使ってください。
...
| 暗号化・identity・メール管理 | ✓ |
| Claude Code セッションログ (llmlog: IngestClaudeCodeLogs / ClaudeCode) | ✓ |
| auto-trigger scheduler 自動起動 (対話 FE のみ、opt-out 可) | ✓ |
...
SourceVault は複数の補助パッケージに分割されています (`SourceVault_core`, `SourceVault_contracts`, `SourceVault_wiring`, `SourceVault_crypto`, `SourceVault_eagle`, `SourceVault_identity`, `SourceVault_maildb`, `SourceVault_mailstructure`, `SourceVault_mailsuggest`, `SourceVault_mcp`, `SourceVault_mining`, `SourceVault_promptrouter`, `SourceVault_searchindex`, `SourceVault_searchview`, `SourceVault_servicemanager`, `SourceVault_simrun`, `SourceVault_webingest`, `SourceVault_workflowcatalog`, `SourceVault_workflowregistry`, `SourceVault_llmlog`, `SourceVault_autotrigger`)。本体は SourceVault です。

新しく分割・追加された主な補助パッケージ:

- **`SourceVault_llmlog`** — Claude Code セッションログ (llmlog) の取り込み・検索 (`SourceVaultIngestClaudeCodeLogs` / `SourceVaultClaudeCode`、Part O 参照)
- **`SourceVault_autotrigger`** — auto-trigger scheduler (`SourceVaultAutoTriggerStartScheduler`)。SourceVault ロード時に対話 FE カーネルでのみ自動起動 (事前準備のメモ参照)
- **`SourceVault_contracts` / `SourceVault_wiring`** — パッケージ間の契約・配線レイヤ
- **`SourceVault_mailstructure` / `SourceVault_mailsuggest`** — メールの構造化・返信提案 (`SourceVaultMailFetchNew` / `SourceVaultMailComposeReply` 等)
- **`SourceVault_simrun`** — シミュレーション実行連携
- **`SourceVault_searchview`** — 検索結果の表示ビュー