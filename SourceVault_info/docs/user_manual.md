...
| 機能 | 内容 |
|---|---|
| コアサブファイルの自動ロード | `SourceVault_core.wl` / `SourceVault_simrun.wl` / `SourceVault_searchindex.wl` / `SourceVault_servicemanager.wl` / `SourceVault_webingest.wl` / `SourceVault_mcp.wl` / `SourceVault_llmlog.wl` / `SourceVault_mailsuggest.wl` を自動ロード |
...
> **自動トリガスケジューラの自動起動:** SourceVault をロードすると、実行環境が Front End のメインカーネル (`$FrontEnd =!= Null`) の場合に限り `SourceVaultAutoTriggerStartScheduler[]` が自動的に呼ばれます。これは「他 PC から『このマシンでこのワークフローを実行して』と依頼されたジョブを、このマシンが常に拾えるようにする」ためのものです。SourceVault.wl はサブカーネル・wolframscript の外部ジョブ・SourceVault サービスカーネル・MCP ゲートウェイカーネルなど、多くのプロセスからロードされますが、スケジューラは **1 マシンにつき 1 箇所 (対話的 FE) だけ**で起動するようガードされています。すべてのカーネルで無条件に起動すると、Wolfram ライセンスの同時カーネル席を浪費し、ジョブが多重ディスパッチされてしまいます。起動は冪等 (`StartScheduler` は同じ tick 登録を再登録するだけ) で、結果は `SourceVault\`Private\`$iSVAutoTriggerSchedulerAutoStartResult` に記録されます (同一カーネルセッション内では 1 回のみ実行)。
>
...
> ```
>
> 成功時は `SourceVaultAutoTriggerStartScheduler[]` 自体の戻り値 (`Association`) がそのまま入ります。
...
SourceVault の暗号・メール機能は、本体 `SourceVault.wl` のローダが依存順に Get する **4 つのサブファイル**に集約されています。また、`Get["SourceVault.wl"]` 単体でのロード時には、コア機能 (`SourceVault_core.wl`)・検索インデックス (`SourceVault_searchindex.wl`)・サービスマネージャ (`SourceVault_servicemanager.wl`) の 3 つに加え、シミュレーション実行基盤・PromptRouter 拡張・Web ingest・MCP・Claude Code セッションログ・メール提案のサブファイルが自動でロードされます。
...
SourceVaultWebImportance[recSourceVaultWebImportance[recordId]            (* Priority + CurrentImportance → CombinedScore *)
...
- **同じ理由で、SourceVault_autotrigger のスケジューラも FE メインカーネル 1 箇所でしか起動しません。** 前述（「ロード時に有効になる機能」節）のとおり、SourceVault.wl はサブカーネル・wolframscript ジョブ・サービスカーネル・MCP ゲートウェイカーネルなど多数のプロセスからロードされるため、どのカーネルでもスケジューラを起動すると同じ理由でライセンス席とジョブディスパッチが多重化します。`$FrontEnd =!= Null` のカーネルだけが起動するようガードされています。
...
Claude Code のセッションログ（実行ログ）を `SourceVault_llmlog.wl` 経由でソースとして取り込みます。取り込まれたログは provider `"claudecode_sessions"` として `SourceVaultSummaries` の横断検索に相乗りし、`GitHubCommitLog`（コミット履歴）や GitHub リポジトリ検索とは別種別として扱われます。
...
> 補助 API ドキュメント (`api_llmlog.md`) は、タスクに「Claude Code」「実行ログ」「セッションログ」「作業ログ」「過去のセッション」等のキーワードが含まれるときのみ注入されます（詳細は「補助 API の条件付き注入」節を参照）。単独の「ログ」だけでは over-match するためトリガーには含まれません。
...
登録済みの provider 集合は `$SourceVaultSummaryProviders`（`name -> fn` の `Association`）で確認できます。`SourceVault_llmlog.wl` の `"claudecode_sessions"` provider もこの仕組みで登録されています。
...
$SourceVaultSummaryProviders   (* → <|"sources" -> ..., "eagle" -> ..., "claudecode_sessions" -> ..., "myprovider" -> ...|> *)
```
...
(* {"sources", "eagle", "claudecode_sessions", ...} *)
```
...
現在のマシンの実測プロファイルを返します（セッション内 memoize）。

**シグネチャ:**
```mathematica
SourceVaultMachineProfile[opts]
```
...
| オプション | 既定 | 説明 |
|---|---|---|
| `"Refresh"` | `False` | `True` で再実測 |

**例:**

```mathematica
SourceVaultMachineProfile[]
(* → <|"MachineName", "MachineTag", "OS", "ProcessorCount", "MemoryGB",
       "WolframVersion", "GPUs" -> {<|"Name","MemoryMB"|>...},
       "GPUAvailable", "NvccAvailable", "NvccPath",
       "SubkernelTarget", "ProbedAtUTC"|> *)
```
...
#### `SourceVaultMachineProfileRefresh` / `SourceVaultMachineSpecs`

`SourceVaultMachineProfileRefresh[]` は現在のマシンを再実測し、共有ストア `<PrivateVault>/machines/<tag>.wl`（Dropbox 同期）へ書き込みます。`SourceVaultMachineSpecs[]` は共有ストアに記録された全マシンのプロファイルを `<|tag -> profile, ...|>` で返します（現在のマシンは常に最新の実測で上書き）。

```mathematica
SourceVaultMachineProfileRefresh[]
SourceVaultMachineSpecs[]
SourceVaultMachineSpecsView[]   (* Dataset 表示版 *)
SourceVaultMachineSpecsText[]   (* 仕様生成プロンプト注入用の compact テキスト表 *)
```

---

#### `SourceVaultGPUAvailableQ` / `SourceVaultNvccPath` / `SourceVaultCUDARequire` / `SourceVaultCUDACompile`

GPU/CUDA サポート関数群です。詳細は「シミュレーション実行基盤」節を参照してください。

```mathematica
SourceVaultGPUAvailableQ[]
SourceVaultNvccPath[]
SourceVaultCUDARequire[]
SourceVaultCUDACompile["kernel.cu", "ExtraArgs" -> {}, "Force" -> False]
```
...
サブカーネル burst 管理関数群です。

**シグネチャ:**
```mathematica
SourceVaultSubkernelTarget[]
SourceVaultWithSubkernels[body]
SourceVaultWithSubkernels[n, body]
```

`HoldAll`。使えるサブカーネルを起動して `body`（`ParallelMap` / `ParallelTable` 等）を評価し、終了後は自分が起動した分だけ確実に停止します（詳細は「シミュレーション実行基盤」節を参照）。

---

#### `SourceVaultSimRunCreate` / `SourceVaultSimRunFinalize` / `SourceVaultSimRunRecord` / `SourceVaultSimRunFolder` / `SourceVaultSimRuns`

SimulationRun（シミュレーション実行）の作成・確定・参照 API 群です。

**シグネチャ:**
```mathematica
SourceVaultSimRunCreate[slug, params]
SourceVaultSimRunFinalize[run, extra]
SourceVaultSimRunRecord[uriOrRef]
SourceVaultSimRunFolder[uriOrRefOrRunId]
SourceVaultSimRuns[slug]
SourceVaultSimRunRoot[]
```

詳細な使い方・オプション・戻り値の形は「シミュレーション実行基盤」節を参照してください。
...
SourceVaultIngestClaudeCodeLogs     Claude Code セッションログの ingest (claudecode_sessions provider)
...
シミュレーション実行基盤            マシンプロファイル共有 (SourceVaultMachineSpecs)・GPU/CUDA サポート・サブカーネル burst 管理
SimulationRun 記録                  実行フォルダ (Dropbox udb/simruns) + メタデータ immutable snapshot の 2 層記録
...
SourceVaultSummaries                Eagle/arXiv/llmlog 等の登録 provider 横断サマリー検索・統合表示
...
$ClaudePackageAuxKeywordMap         補助 API の条件付き注入 (メール/arXiv/llmlog はキーワード一致時のみ)
SourceVault_llmlog.wl               Claude Code セッションログの ingest サブファイル (自動ロード)
SourceVault_simrun.wl               シミュレーション実行基盤サブファイル (自動ロード)
自動トリガスケジューラの自動起動    FE メインカーネルロード時のみ SourceVault_autotrigger スケジューラを冪等に自動起動
...
(* CUDA 前提検査してからサブカーネル burst で並列実行 *)
If[SourceVaultCUDARequire[]["OK"],
  SourceVaultWithSubkernels[ParallelTable[i^2, {i, 1, 100}]]]

(* SimulationRun の作成・確定・履歴参照 *)
run = SourceVaultSimRunCreate["demo-run", <|"N" -> 100|>];
result = SourceVaultSimRunFinalize[run, <|"Status" -> "Done"|>];
SourceVaultSimRuns["demo-run"]
