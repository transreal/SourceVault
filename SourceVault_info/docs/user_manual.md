...
| 機能 | 内容 |
|---|---|
| コアサブファイルの自動ロード | `SourceVault_core.wl` / `SourceVault_searchindex.wl` / `SourceVault_servicemanager.wl` を自動ロード |
...
SourceVault の暗号・メール機能は、本体 `SourceVault.wl` のローダが依存順に Get する **4 つのサブファイル**に集約されています。また、`Get["SourceVault.wl"]` 単体でのロード時には、コア機能・検索インデックス・サービスマネージャの 3 つのサブファイルが自動でロードされます。
...
- `SourceVault_mcp.wl` — MCP tool schema・dispatch（protocol endpoint は Python proxy 側）。
...
deterministic な notebook クエリ。検索スコープは `$SourceVaultDefaultNotebookFolder` で制御されます。
...
登録済み全 source のメタ情報を対話的な統合表で表示します (FrontEnd 必須)。各行のタイトルはクリック可能で、クリックすると 1 件のフルメタ情報が別ウィンドウで開きます。`"▶ 開く"` ボタンでソースファイルまたは URL を直接開けます。
...
Eagle 保存済みサマリー・arXiv 論文・登録 provider を横断して検索し、統合表で表示します (FrontEnd 必須)。arXiv source については**タイトル・著者・出版日を arXiv API から自動取得してキャッシュ**し、表の `Authors` 列に表示します。各行のタイトルをクリックするとフルメタ情報が別ウィンドウで開きます。
...
```mathematica
(* 全 provider の横断サマリーを表示 *)
SourceVaultSummaries[""]

(* 論文関連を横断検索 (arXiv Authors が自動補完される) *)
SourceVaultSummaries["量子コンピュータ"]

(* ネットワークなし (キャッシュ済みメタのみ) *)
SourceVaultSummaries["", "FetchMetadata" -> False]

(* Dataset として取得して Authors 列を利用 *)
ds = SourceVaultSummaries["", "Format" -> "Dataset"];
ds[Select[#Authors =!= {} &]]
```

> **arXiv メタの自動取得:** `FetchMetadata -> Automatic` (既定) のとき、meta に `Authors` が未登録の arXiv source を検出して `iSVArXivMetaFetchBatch` で一括取得し、snapshot record にキャッシュします。次回以降は再取得しません (`FetchMetadata -> True` で強制再取得できます)。バッテリー節約・ローカル LLM 環境など network を使いたくない場合は `FetchMetadata -> False` を指定してください。
...
SourceVaultSources          登録 source 全件の対話的統合表示 (タイトルクリック・開くボタン)
...
SourceVaultSummaries["arXiv", "FetchMetadata" -> Automatic] (* arXiv Authors を補完取得 *)
