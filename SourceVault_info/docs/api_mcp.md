...
- `sourcevault_fs_read` {path, maxBytes?} — allow-list root 配下の UTF-8 text file を読む。実体 `iSVFSReadFile`。`.env`/`*secret*`/`*credential*`/鍵・証明書ファイル/`.ssh` 等は拒否、size は既定 512 KiB (`$svFSMaxBytes`) を上限に clamp (`maxBytes` で更に小さくは指定可)。
...
- `sourcevault_workflow_write` {path, content} — allow-list root の `SourceVault_workflows/` サブツリーへ UTF-8 text を書き込む (上書き・親ディレクトリ自動作成・1 MiB 上限 `$svFSMaxWriteBytes`)。実体 `iSVFSWriteFile`。file 編集不可な API/LM Studio クライアント向け (Claude Code/Codex には不要)。write のため `iSVRecordToolCall` の観測ログ対象外。
...
**scope (負荷制御)**: 全コーパス (161 ファイル / 数千 session) を service カーネルで on-demand build すると重すぎる (session index = per-session TopicEnrichment)。既定 `SourceVault`$svOOPSMCPScope = {"oops 200506.txt"}` の bounded scope に絞る (この既定値は SourceVault_mcp.wl 自身が定義する)。初回 tool 呼び出しで ~10s (quote-table parse 込み) → 以降 cache。広げるには service カーネルで `$svOOPSMCPScope` を設定 (再起動 or `SourceVaultOOPSEnsureLoaded["Force"->True]` で反映)。
...
- **`sourcevault_mail_status`** — scope (`SourceVault`$svMailStructMCPScope = {mbox, period}`、既定 `{"univ", "202606"}`。この変数は SourceVault_mailstructure 側で定義され、本ファイルには存在しない) のメールをロード → StructureMail → BM25 index を cloud PrivacyScope で lazy build し状態 (`Loaded / Scope / MailCount / SessionCount / VocabSize / IndexId`) を返す。検索前に一度呼ぶ。実体 `SourceVaultMailStructEnsureIndex`。
...
### 組み込み adapter (SourceVault_mcp.wl ロード時に自動登録)
...
- `mail` — Kinds {"mail"}。main-kernel only (`SourceVaultMailSearchIndex` の DownValues 有無で probe)。Capabilities: Search/ReadMetadata/ReadSummary/MetadataFilter/ResolveObjectURI/ReadBody。RequireGrantFor {"body","raw","attachment"}。`sv://record/svmail-...` を索引から解決 (本文は grant 済みでのみロード)。
- `filesystem` — Kinds {"file","filesystem"}。allow-list root 配下の読み取り専用ブリッジ (`sourcevault_fs_list`/`sourcevault_fs_read` と同じ allow-list)。Capabilities: ReadMetadata/ReadSummary/ResolveObjectURI。RequireGrantFor {} (PrivacyLevel 0.0 = cloud-ok)。
...
model 別 AccessProfile を PrivateVault/config/mcp-access-profiles.json に永続化する (AccessProfileRef 方式・GitHub 非公開)。位置引数のみ (Options は取らない)。modelId に "*" を渡すと provider/intent の wildcard profile。
profile key と未指定時の既定値: MaxAccessLevel (trust domain が Cloud/Unknown なら 0.49、それ以外 1.0) / AllowedSinks ({"LocalOnly","MCPResponse"}) / AllowedOperations ({"Search","ReadSummary","ReadContext"}、DepositArtifact は含まれない=fail-closed) / AllowedProjections ({"metadata","summary","snippet"}) / DeniedProjections ({"raw","downloadOriginal"}) / ScopePolicy / PurposeAllowed (All) / RequireGrantFor ({"body","raw","notebookCell","mailBody"}) / Audit (`<|"LogLevel"->"DecisionOnly"|>`) / EndpointRef / TrustDomain。返値は正規化済み profile。PrivateVault 未解決/書込失敗は Failure。
...
request に対する実効 AccessProfile を解決する。解決順: exact (provider:intent:modelId) → wildcard (provider:intent:* / provider:*:*) → default。default は trust-domain cap (cloud 0.49 / local 1.0) と安全側 ScopePolicy (`DenyAccessTags -> {"NoExternal","StudentPrivate","Personal"}`、他は登録済み profile と同じ既定値) で、profile 未登録時に既存 ceiling を変えない。
...
LocalState/secrets/sourcevault-grant-signing-key.json に grant 署名用 shared secret (HMAC-SHA256 鍵、32byte) が無ければ生成する (main↔service 共有・MCP transport token とは別物)。既定 KeyId "grant-hmac-v1"。
...
- kind / mediaType / content (commit時。text または `<|"encoding"->"base64","data"->...|>`。既定 encoding は "utf-8" で `content.text` を使う)
...
その他失敗 Status: "InvalidContent"(base64 decode失敗) / "PayloadTooLarge"(Bytes/MaxBytes=5MB 超過) / "EmptyText" / "QuotaExceeded"(session/batch あたり件数 200 or 容量 50MB 超過。Usage/MaxItems/MaxBytes を返す) / "BlobCommitFailed"(Detail) / "DepositWriteFailed"(Detail) / 不正 mode は `Failure["InvalidDepositMode"]` (Status キーを持つ通常の Association ではなく生の Failure オブジェクトなので JSON 化経路が他と異なる点に注意)。
