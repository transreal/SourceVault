...
Commit path (single serialized writer): journal `Prepared` → index dirty marker → atomic record write (`Revision`+1, `Writer -> <|MachineTag, Epoch|>`, `UpdatedAt`) → index row update → marker clear → journal `Completed`. Index write failure still leaves the record commit valid; the dirty marker forces a later rebuild. Index file is `index.wxf` v2 (`IndexSchemaVersion = 3`, `<|"IndexSchemaVersion", "BuiltAtUTC", "AppliedOperationId", "Rows"|>`); legacy flat indexes are rebuilt on first commit.
