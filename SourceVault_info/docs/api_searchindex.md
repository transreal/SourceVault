...
release context gate 付きで検索し Permit のみの SearchResult リストを返す (§7.4)。raw local path は返さない。request-time gate を再評価する。`"Index"` 指定時は native projection index を使い PDFIndex 非依存。
Options: `"ReleaseContext" -> None`（必須）, `"PDFIndexProfile" -> None`, `"Collection" -> Automatic`, `"Limit" -> 20`, `"Index" -> None`（native index ID）
SearchResult スキーマ: `<|"ResultId", "ChunkId", "Score", "Snippet", "EvidenceRef", "Citation" -> <|"Title", "Page", ...|>, "ReleaseDecision" -> "Permit", "RequestTimeGateReevaluated" -> True, "PolicyDigestAtRequest", "Why"|>`
例: `SourceVaultSearch["量子コンピュータ", "ReleaseContext" -> "public", "Index" -> "my-proj", "Limit" -> 10]`
...
chunk 群に build-time release gate を適用し Permit のみの KeywordBigram projection index を作り immutable 保存する (§6.3)。
→ `<|"Status" -> "OK", "IndexId", "Ref", "ChunkCount" -> _Integer, "ExcludedCount" -> _Integer|>`
Options: `"Chunks" -> None`（必須: §7.2 chunk のリスト）, `"IndexId" -> Automatic`（既定: contextName <> `"-proj"`）
例: `SourceVaultBuildProjectionIndex["public", "Chunks" -> chunks, "IndexId" -> "pub-v1"]`
...
ロード済み: `<|"IndexId", "Loaded" -> True, "ChunkCount", "ReleaseContextRef", "IndexKind" -> "KeywordBigram"|>`
