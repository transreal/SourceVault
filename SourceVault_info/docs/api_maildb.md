...
ルーチンアジェンダ (`SourceVaultRoutineAgendaView` / `SourceVaultRoutineAgendaData` の
✉ 要対応メールバンド = 作業依頼/出席依頼/確認・〆切ありのオーナー宛てメール) が
**既に拾っているメールを一覧から除外**する。アジェンダで対応するメールを一覧でもう一度
チェックする二度手間を無くすためのオプションで、上記すべての検索/一覧/View 関数
(`SourceVaultSearchMailSnapshots` / `MailSearchSummary` / `MailDataset` / `MailView` /
`MailSearchIndex` / `MailSearchIndexView`) が同じオプションを共有する。

| "ExcludeAgenda" | 動作 |
|---|---|
| `False` (既定) | 除外なし = 従来動作 |
| `True` | アジェンダ項目とその**スレッド全体** (ThreadRecordIds) を除外 |
| `"Item"` | 代表メールだけを除外 (同スレッドの他の便は一覧に残す) |
| `{rid, ...}` | 明示指定した RecordId を除外 (テスト注入・任意の除外) |

`"AgendaItems"` (既定 `Automatic`) はアジェンダ項目の供給源。`Automatic` は
`SourceVaultMailAgendaItems[]` をライブ計算 (索引だけを読む軽量パス) し、結果を
`$SourceVaultMailAgendaExcludeCacheTTL` 秒 (既定 60、`0` で無効) だけ使い回す。
アジェンダを表示した直後なら `SourceVaultMailAgendaItems[]` や
`SourceVaultRoutineAgendaData[]` の戻り値をそのまま渡せば再計算しない
(`<|"Items"->{...}|>` / `<|"Mail"->{...}|>` / 素の item リスト いずれも可)。

除外は **Limit 適用より前**に行うので、`"Limit"->50` の枠が除外分で食われることはない。
mailagenda 層へは弱結合 (rule 11): 未ロード・計算失敗時は除外なしで全件返る。
...
- **Core 系** (行データを返す): `SourceVaultSearchMailSnapshots` / `SourceVaultMailSearchIndex` は返り値の最大 `PrivacyLevel` を `SourceVaultNotePrivacyOf` で透かしに立てる。**値の形は変えない**。
- **View 系** (表示オブジェクトを返す): `SourceVaultMailView` / `SourceVaultMailSearchIndexView` は `SourceVaultPrivateView` を通り、PL ≥ 0.5 なら `SourceVaultPrivate[Pane[...], pl]` に包まれる (赤枠 + 「機密 / Confidential PrivacyLevel x.xx」バッジ)。構造を検査するコード/テストは `SourceVaultPrivacyUnwrap` を通すこと。
  ⚠️ **View 用バッジを Core 系に付けてはいけない** — `Dataset[SourceVaultMailSearchSummary[...]]` のような下流が壊れる。
- **機密生成ヘッド表** に索引 sidecar 経路を追加した (以前は丸ごと未登録だった): `SourceVaultMailSearchIndex` / `SourceVaultMailSearchIndexView` / `SourceVaultMailIndexGet` / `SourceVaultMailThreadNotebook` / `SourceVaultMailShowBody`。
...
### $SourceVaultMailAgendaExcludeCacheTTL
型: Integer, 初期値: 60
`"ExcludeAgenda"->True` の一覧関数がルーチンアジェンダ項目をライブ再計算せず使い回す秒数。`0` で毎回再計算 (キャッシュ無効)。
...
**2 列目以降の埋め込みボタンは当たり判定が描画とズレて押せなくなる**（幅を広げても不可、
`{2,幅}` / `{Automatic,幅}` どちらでも再現、実機確定）。列幅を捨てると右側の列がはみ出すので、
幅は `ItemSize` で確保しつつ、クリックは当たり判定が生きている 1 列目に集約する。

`SourceVaultMailView` / `SourceVaultMailSearchIndexView` の列幅（**文字幅単位、px ではない**）。
未登録の列や `Automatic` は自動幅。

**⚠️ Dataset の `ItemSize` は `{行, 列}` の順**（Dataset の階層順。`Grid` の `{列, 行}` とは逆）。
旧 `showMails` の `ItemSize -> {2, 列幅リスト}` が正しく効いていたのが根拠。逆順の
`{列幅リスト, Automatic}` にすると**列幅が `Automatic` 扱いになり、Dataset が列を Pane 幅いっぱいに
引き伸ばして右側の列 (件名/差出人/要約) がはみ出して見えなくなる**（実際に発生した事故）。
なお**セルを `Pane` で包む方法は無効** — Pane は中身をクリップするだけで列自体は縮まない（実機確認済み）。
列構成を変えたら `iSVUIItemSize[{列 ID...}]` の並びも必ず合わせ、✉/スレ ボタンのクリックを実機確認すること。

例（要約を狭めて件名を広げる）:
`$SourceVaultMailViewColumnWidths = Join[$SourceVaultMailViewColumnWidths, <|"Subject"->34, "Summary"->28|>]`
