...
Options: "MBox" -> Automatic (文字列でその mbox に限定 / Automatic=ロード済み全 mbox), "Limit" -> 50 (Infinity で範囲内全件), "DateFrom" -> Automatic, "DateTo" -> Automatic (DateObject/文字列/{y,m,d} で対象を日付範囲に限定、日単位包含), "Refresh" -> None (None=Pending のみ / "MissingCategory"=Category 未生成の処理済み旧 snapshot も再処理 / All=全件 / Function=述語一致を再処理), "Inferencer" -> (実LLM, 注入可), "CheckpointEvery" -> 20, "Persist" -> True
