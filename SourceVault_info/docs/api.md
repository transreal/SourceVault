...
Options: `"Roots"` -> Automatic (`$SourceVaultRoots` を override), `"Force"` -> False (True で再初期化), `"InitMode"` -> "Force" (既定、本来の初期化) | "Ensure" (初期化済みなら副作用ゼロで AlreadyInitialized。依存 DAG 経由の冪等実行は `SourceVaultEnsureInitialized` — api_contracts.md 参照)
