...
OCR hook を有効化する。backend 既定 `"ClaudeVision"`。全 backend は単一の共有 Options プール (`"DPI"`, `"SplitHalves"`, `"Timeout"`, `"Language"`, `"Prompt"`, `"Hook"`, `"Mode"`, `"Verbose"`) を持つ (backend ごとの個別既定値の切替はない)。
→ `<|"Status" -> "Enabled", "Backend" -> _String, "Mode" -> _String, "Options" -> _Association|>`
backend:
- `"ClaudeVision"`: `ClaudeCode`ClaudeQueryBg` 経由で Claude API にページ画像を送り OCR (PDFIndex 実証済みパターン)。大きいページは自動で上下分割 (30px overlap) して 2 回 OCR しマージ。関連 Options: `"DPI"` -> 300, `"SplitHalves"` -> True, `"Timeout"` -> 180, `"Prompt"` -> Automatic
- `"TextRecognize"`: Mathematica 組み込み TextRecognize (Python 不要、精度準等)。関連 Options: `"DPI"` -> 300 (共通既定), `"Language"` -> `"Japanese"`
- `"Custom"`: ユーザー提供 Function をそのまま `$SourceVaultOCRHook` に設定。関連 Options: `"Hook"` -> `Function[req, text]`
