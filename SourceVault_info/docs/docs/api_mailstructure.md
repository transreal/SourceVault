...
**topic 品質フィルタ**: salience gate は低頻度（`< *Min`）に加え、**遍在語**（`DistinctMails > MaxDocFreqFraction × メール数`＝多数メールに出る語は discriminating でない）と**汎用メール語 stoplist**（詳細/送信/確認/アカウント/address/subject 等）も除外する。
Options: `"PrivacyScope"`, `"GroupingKind"`(`Mail`|`PreliminaryThread`|`Session`, 既定 Mail), `"Sessions"`(mailRef→sessionId。pass B で DistinctSessions salience), `"DistinctMailMin"`(2)/`"DistinctThreadMin"`(2)/`"DistinctSessionMin"`(2), `"MinSupport"`(閾値一括上書き), `"MaxDocFreqFraction"`(0.5。遍在語上限), `"TopicStoplist"`(Automatic=内蔵汎用語), `"MaxNewTopics"`(200), `"OwnerRef"`, `"Rounds"`(1), `"PerMailLimit"`(40), `"CandidateBlockFilters"`(既定 {Quote,Signature,Footer})、`"ImportRunId"`(Automatic=ProfileDigest から導出する run id)、`"Extractor"`(Automatic=決定論抽出器 `ExtractCandidateTopics` / 関数=注入抽出器。詳細は Inc6)、`"CandidateSource"`("AutoExtracted"。Provenance.Source に反映。詳細は Inc6)。
...
Options: `"QuotePass"`(`LocalOnly`|`IndexBuildOnly`|`Full`, 既定 LocalOnly。LocalOnly は GlobalQuoteIndex pass の edge を落とす), `"QuoteIndex"`(既存 index 再利用), `"MaxContinuationGapDays"`(30), `"MaxGlobalCandidatesPerQuote"`(20), `"MaxQuoteBlocksPerMail"`(40), `"MaxAmbiguityForMerge"`(2), `"MinLineJaccardExact"`(0.6), `"MinShingleJaccardFuzzy"`(0.3), `"LocalWindowDays"`(14), `"PrivacyScope"`, `"ProfileDigest"`, `"SubjectStoplist"`(Automatic=内蔵定型件名 stoplist。"会議"/"確認"/"資料"等を subject fallback edge 生成対象から除外)。
戻り値（graph 本体）: `<|"GraphId", "BuildId", "ProfileDigest", "PrivacyScope", "QuotePass", "Nodes", "Edges", "QuoteIndexBuildId"|>`（`Edges` が下記 edge 列）。
...
戻り値: `<|"Vocabulary"(ObservedRelationGraph 込み), "RelationGraph", "Sessions", "TopicGraph", "ParagraphTopics", "Records", "Report"|>`。Options: `"Seed"`(dict|None), `"Grow"`(True), `"PassB"`(True), `"QuotePass"`(Full), `"PrivacyScope"`, `"OwnerRef"`, `"MaxTopicsPerMail"`(8), `"VocabOptions"`(pass A/B の `GrowTopicVocabulary` へ透過する opts。例 `{"MaxNewTopics"->120, "MaxDocFreqFraction"->0.3, "DistinctMailMin"->3}` で vocab を絞る)。
**注**: `Report["VocabSize"]` は `MaxNewTopics`(pass 毎, 既定 200) × 2 pass ＝ 最大 400 で頭打ちになる。vocab を小さく/鋭くするには `VocabOptions` で `MaxNewTopics` を下げる・`MaxDocFreqFraction` を厳しく・`DistinctMailMin` を上げる。
