...
不変スナップショットの privacy level を返す。privacy サイドレコードがあればその `Level`、無ければ `$SourceVaultDefaultObjectPrivacyLevel` (0.85)。read-only。変更は `NBAccess`NBSetSnapshotPrivacyLevel` 経由のみ。
...
→ `<|"Status" -> "OK"|"Failed", "Ref" -> ref, "OldPrivacyLevel" -> old, "NewPrivacyLevel" -> new, "Lowered" -> True|False, ...|>`
...
`SetAttributes[HoldRest]`。lock を取得して body を評価し、必ず lock を解放する。取得できなければ `Failure["LockTimeout", ...]` を返す。body は lock 取得まで評価されない。
→ body の評価結果 | Failure
...
`SourceVaultTryLock` が返した `Handle` Association を渡して lock を解放する。他プロセスの lock は解放しない (Owner 同一性確認)。
...
event を append-only event directory に one-event-one-file で commit する。`EventID` が無ければ `"evt:"<>CreateUUID[]` を付与。`Digest` と `CreatedAtUTC` を補う。同一 `EventID` の再 commit は digest 一致なら idempotent 成功、不一致なら `"EventIdCollision"`。
...
未参照かつ retention 期限切れの blob を回収する。grace period 内の blob / in-flight write / replay 失敗 vault は削除しない (fail-closed)。実削除前に `SourceVaultCheckVaultConsistency` を実行し、不健全な vault では GC しない。
