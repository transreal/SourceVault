...
| 観測常時化 | **永続観測設定+ロード時自動適用**(§1.14) | capbroker.wl+SourceVault.wl | 73/73 | result5.nb ✅ |
| GC | **prepared/lease TTL prune**(§1.15) | capbroker.wl+servicemanager.wl | 34/34 | — |
| anomaly stream | **実 event → rate stream collector**(§1.15) | anomaly.wl | 80/80 | — |
...
**GitHub コミット済み(af94945、blob 5)+ NB 実機済み(result5.nb)**: owner が
`SourceVaultSetBoundaryObservation[<|"Shadow"->True, "OwnerInputShadow"->True|>]` を実行し
**この機械(ProArt)で観測の常時化が実運用開始**(Config/Live とも両 True を確認。
UpdatedAtUTC 2026-07-15T00:45Z)。以後この機械では SourceVault ロード時に全カーネルで観測が自動点灯。
他の機械(rapterlake4t 等)は各機械で 1 回 Set が必要(LocalState=機械ローカル・非同期)。
...
### 1.15 観測常時化の副作用手当て: ledger GC + anomaly 実 stream(2026-07-15)

観測を常時 on にしたことで生じる 2 つの実運用課題を手当て。

**(1) prepared/lease ledger GC**: capbroker に `SourceVaultPruneCapBroker[opts]`(**DryRun 既定 True**=
rule 103。prepared=consumed/期限切れ、lease=consumed|revoked かつ期限切れを削除。**issued の未期限
lease は残す**=実行中権限を消さない。GraceSeconds/PreparedOnly)。servicemanager の service ループに
低頻度自動 prune 結線($SourceVaultCapBrokerPruneIntervalSeconds 既定 3600・安全範囲を DryRun->False+
Grace 300 で自動)。shadow/warn の非消費 verify と self-prepare で溜まる issued レコードを掃除。
capbroker_test 24→**34/34**(DryRun 非削除/consumed・expired 削除/fresh 生存/issued lease 保護/冪等)。

**(2) anomaly 実 stream collector**: anomaly に `SourceVaultCollectCaneAnomalyStreams[opts]`
(observe-only event store→rate stream 日次ビン化・LLM 不使用。既定=LLMBoundaryMismatchRate/
GuardMailMisalignedRate、**owner 状態=opt-in で OwnerInputHighRiskRate**。実データ無し stream は
含めない=空捏造しない I-15)。schedule tick の**既定 RunFn を collector 供給に接続**(手動
RunCaneAnomalyWorkflow[] は空のまま=後方互換)。anomaly_test 73→**80/80**。
**罠22: WL の文字列 `>=` は評価されない(GreaterEqual のまま)→ 日付順比較は `OrderedQ` を使う**。
回帰: capbroker 34/34・anomaly 80/80・schedule 14/14・llmshadow 73/73・servicemanager 22/22・
taint 26/26・security 7/7 green。

## 2. 未実装(次セッションの選択肢。優先度順)

(warn/enforce=§1.11(1)、1F 結線=§1.11(2)、anomaly schedule=§1.11(3)、token 配線=§1.12+§1.13、
1G 入口 shadow=§1.13(2)、観測常時化=§1.14、ledger GC+anomaly 実 stream=§1.15 で実装済み)
...
```
期待: 全 green(合計 = 35+32+47 + 43+24+21+16+26 + 24+28 + 34+26 + 7 + 80+14 + 73)。
capbroker は 24→34(prune 10 追加)、anomaly は 73→80(collector 7 追加)、llmshadow は 67→73
(観測常時化 O 節 6 追加)、cognition_ownershadow 26 は 1G'(§1.13)で新規、に増えている点に注意。
