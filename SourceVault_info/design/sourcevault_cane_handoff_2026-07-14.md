...
| 1H-S⑤ | **移行第二段=warn/enforce 段階ゲート**(mode+EnforceList+token 配線パイロット) | capbroker.wl+12ファイル | 60/60 | — |
| 1F'' | **実 proposer 供給**(MakeLLMProposer/Verifier+非同期 SubmitMultiModelDecision) | adjudication.wl | 28/28 | — |
| 1H-A' | **anomaly schedule 配線**(ScheduleTick+service 低頻度 hook 結線・既定 off) | anomaly.wl+servicemanager.wl | 14/14 | — |
...
(旧 1=warn/enforce は §1.11(1)、旧 3=1F 結線は §1.11(2)、旧 4=anomaly schedule は §1.11(3) で実装済み)

1. **boundary の実運用昇格(運用タスク・owner 判断)**
   (a) `$SourceVaultLLMBoundaryShadow = True` を FE init に置いて coverage/mismatch データ収集
   (`SourceVaultLLMBoundaryShadowStats`)。(b) 残り 17 入口への token 配線(パイロット=SummarizeText 済み。
   各呼び出し元で `SourceVaultPrepareLLMInput`→境界へ PreparedToken を渡す)。(c) 配線済み入口から
   `$SourceVaultLLMBoundaryEnforceList` に追加して個別 enforce(全体 `$SourceVaultLLMBoundaryMode="Enforce"`
   は全配線後)。機構は §1.11(1) で完成済み・コード変更不要の運用昇格。
