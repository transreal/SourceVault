...
1. **boundary の実運用昇格(運用タスク・owner 判断)**
   ...
   (`SourceVaultLLMBoundaryShadowStats`)。(b) 重要 chain の上流 mint 化(self-prepare より早い束縛点へ。
   SummarizeText→iWebLLMComplete が先行例)。(c) 任意の入口を `$SourceVaultLLMBoundaryEnforceList` に
   追加して個別 enforce(§1.12 で全入口が昇格可能)。全体 `$SourceVaultLLMBoundaryMode="Enforce"` は
   運用実績を見て owner 判断。機構は完成済み・コード変更不要の運用昇格。
