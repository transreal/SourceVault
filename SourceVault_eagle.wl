...
$SourceVaultEagleCloudPublishableTag::usage = "$SourceVaultEagleCloudPublishableTag はクラウド要約を許可するタグ名 (既定 \"Cloud-Publishable\"、大文字小文字無視)。このタグが付いた item は \"Method\"->Automatic の要約でローカル LLM ではなくクラウド経路を使い、summary record に PrivacyLevel 0.0 が記録される。また summary record に PrivacyLevel が無くても実効 PL に $SourceVaultEagleCloudPublishablePL (既定 0.45) の上限が適用され、MCP release gate (sourcevault_get 等) でクラウド開示可になる。クラウド経路は $ClaudeModel の provider 準拠 (claudecode→Claude Code CLI、codex→Codex CLI。課金 API は anthropic/openai 明示時のみ)。また、このタグ付き item のサマリーノート (notes/*.nb) では NBMarkCellConfidential で秘匿マークされたセル (confidential=True または privacyLevel>0.5) を検索インデックス/メタ情報 (Note フィールド) から除外する — サマリー等のメタ情報を Cloud-Publishable な PL に保つため。タグ無し item のノートは全文がローカル検索対象 (メタ情報はライブラリ既定 PL の fail-safe マーキングが前提)。";
...
$SourceVaultEaglePrivacyLevel::usage = "$SourceVaultEaglePrivacyLevel は Eagle View/Dataset/Search/GeoView 出力セルの既定 PrivacyLevel。数値 (全ライブラリ共通)、または <|登録名orライブラリパス -> PL, \"Default\" -> PL|>。既定 1.0 (クラウド LLM へはスキーマのみ・ローカル LLM へは全文)。0.5 以下ならクラウドにも全文可としてマークしない。Cloud-Publishable タグ付き item は $SourceVaultEagleCloudPublishablePL の上限が適用される。";
$SourceVaultEagleCloudPublishablePL::usage = "$SourceVaultEagleCloudPublishablePL は Cloud-Publishable タグ ($SourceVaultEagleCloudPublishableTag) 付き item の実効 PrivacyLevel 上限 (既定 0.45 = クラウド可)。summary record に per-item の PrivacyLevel を持たない item に 実効 PL = Min[ライブラリ既定, この値] として適用する。record の PrivacyLevel (SourceVaultEagleSetSummaryPrivacy / クラウド要約の 0.0) はこの上限より優先。非数値を設定するとタグの PL 反映を無効化 (従来挙動 = ライブラリ既定のみ)。タグ無し item には一切影響しない (fail-closed)。";
...
SourceVaultEagleExtractText[itemSpec_, OptionsPattern[]] :=
  Module[{lib = iSVEGLib[OptionValue["Library"]], item, path, kind, text,
      maxChars = OptionValue["MaxChars"], maxPages},
    (* "MaxPages" は整数のほか All / Infinity (= 全ページ) を受ける。
       内部経路 (iSVEGPdfText) は maxPages_Integer なので大整数に正規化する。 *)
    maxPages = OptionValue["MaxPages"];
    maxPages = Which[
      IntegerQ[maxPages] && maxPages > 0, maxPages,
      maxPages === All || maxPages === Infinity, 10^6,
      True, 15];
    If[lib === $Failed, Return[iSVEGNoLib[]]];
    item = iSVEGItemOf[lib, itemSpec];
    If[item === $Failed, Return[<|"Status" -> "Error", "Reason" -> "ItemNotFound"|>]];
    ...
    text = Switch[kind,
      "PDF", iSVEGPdfText[lib, item, path, maxPages],
...
(* WL 15.0 の Import[..,"Exif"] はネスト形式
   <|"GPSInfo"-><|GPSLatitude..|>, "Image"-><|Make..|>, "Photo"-><|DateTimeOriginal..|>,
     "Thumbnail"->..|> を返す (GPSInfo 内のキーは従来通り GPS 接頭辞付き)。
   グループを flatten して旧フラット形式 (トップレベルに Make/GPSLatitude..) へ正規化。
   旧形式はグループ association を含まないのでそのまま通る。 *)
iSVEGExifFlatten[e_Association] :=
  Module[{groups = Select[KeyTake[e, {"Image", "Photo", "GPSInfo"}], AssociationQ]},
    If[Length[groups] === 0, e,
      Join @@ Prepend[Values[groups],
        KeyDrop[e, {"Image", "Photo", "GPSInfo", "Thumbnail"}]]]];
iSVEGExifFlatten[e_] := e;

iSVEGExtractExifAssoc[path_String] :=
  Module[{e = Quiet@Check[Import[path, "Exif"], $Failed]},
    Which[
      AssociationQ[e], KeyTake[iSVEGExifFlatten[e], $iSVEGExifKeys],
      MatchQ[e, {__Rule}], KeyTake[iSVEGExifFlatten[Association[e]], $iSVEGExifKeys],
      True, $Failed]];
...
(* item の実効 PrivacyLevel: summary record の "PrivacyLevel" があればそれ、
   ...
   (iSVEGDefaultPLFor) を適用。SourceVaultEagleSummaryRow と同じ規則。 *)
iSVEGItemPLOf[lib_, sm_, item_ : None] :=
  With[{p = If[AssociationQ[sm], Lookup[sm, "PrivacyLevel", Missing[]], Missing[]]},
    If[NumericQ[p], N[p],
      iSVEGDefaultPLFor[item, If[StringQ[lib], iSVEGLibraryPL[lib], 1.0]]]];
          ...
          <|"Date" -> Style[iSVEGDateStr[Lookup[it, "btime", Missing[]]], FontFamily -> ff],
            ...
            "Summary" -> iSVEGMemoCell[
              Which[
                ...
                True, ""]],
            (* 実効 PrivacyLevel (record PL > Cloud-Publishable タグ上限 > ライブラリ既定) *)
            "PL" -> Style[NumberForm[iSVEGItemPLOf[lib, sm, it], {4, 2}], FontFamily -> ff],
      ...
      Function[it,
        Module[{path = SourceVaultEagleItemPath[it, "Library" -> lib], exif, pos, th},
          If[! StringQ[path], Nothing,
            ...
            pos = If[AssociationQ[exif], iSVEGExifPosition[iSVEGExifFlatten[exif]],
              If[ListQ[exif],
                iSVEGExifPosition[iSVEGExifFlatten[Association[exif]]],
                Missing["NoGPS"]]];
...
(* ════════════════════════════════════════════════════════
   ...
   PL の決定規則:
     item PL = summary record の "PrivacyLevel" (per-item 上書き、
               ...
               ただし Cloud-Publishable タグ付き item はライブラリ既定に
               $SourceVaultEagleCloudPublishablePL (既定 0.45) の上限を適用
               (タグ = クラウド可の明示宣言。record PL はタグより優先)
...
(* Cloud-Publishable タグ付き item の実効 PL 上限 (0.45 < 0.5 = MCP release gate の
   cloud 閾値なのでクラウド開示可)。非数値でタグの PL 反映を無効化。 *)
If[! ValueQ[$SourceVaultEagleCloudPublishablePL],
  $SourceVaultEagleCloudPublishablePL = 0.45];

(* record PL を持たない item の既定 PL: Cloud-Publishable タグ (ユーザーの明示宣言)
   があればライブラリ既定にタグ上限を Min 適用、無ければライブラリ既定のまま
   (fail-closed: タグは PL を下げる方向にしか働かない)。 *)
iSVEGDefaultPLFor[item_Association, libPL_?NumericQ] :=
  If[iSVEGCloudPublishableQ[item] && NumericQ[$SourceVaultEagleCloudPublishablePL],
    Min[N[libPL], N[$SourceVaultEagleCloudPublishablePL]],
    N[libPL]];
iSVEGDefaultPLFor[_, libPL_?NumericQ] := N[libPL];
iSVEGDefaultPLFor[___] := 1.0;
...
(* item 1 件の PL: summary record の "PrivacyLevel" 優先、無ければライブラリ既定に
   Cloud-Publishable タグ上限を適用 (iSVEGDefaultPLFor) *)
iSVEGItemPL[item_, libPL_?NumericQ, recIdSet_Association] :=
  Module[{id = iSVEGItemId[item], rec, pl},
    If[! KeyExistsQ[recIdSet, id], Return[iSVEGDefaultPLFor[item, libPL]]];
    rec = iSVEGImportJSON[iSVEGSummaryPath[id]];
    pl = If[AssociationQ[rec], Lookup[rec, "PrivacyLevel", Missing[]], Missing[]];
    If[NumericQ[pl], N[pl], iSVEGDefaultPLFor[item, libPL]]];
iSVEGItemPL[___] := 1.0;
